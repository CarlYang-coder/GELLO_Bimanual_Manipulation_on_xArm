...
- Press ENTER to move arms to training initial pose
- Wait for arms to reach target, then press ENTER to start policy
...
MAX_DQ_GRIP  = 0.2   # gripper ratio per step (CSV dq range ~0.1)
EMA_ALPHA    = 0.8     # lower = less bias accumulation, more responsive
DEADBAND_JOINT = 0.001
DEADBAND_GRIP  = 0.002

import numpy as np  # needed for Q_MIN/Q_MAX below

# ---- Joint position limits (P5/P95 base, L3 tightened) ----
# L2 needs full P5 range (0.034) — the policy legitimately drives L2 from ~0.4 down to ~0.03.
# L3 keeps tightened upper bound (0.45) to prevent upward drift.
# 16D order: left_1..7, left_grip, right_1..7, right_grip
Q_MIN = np.array([
    -0.049088, 0.0338, -0.046004,  0.292987, -0.378898,  0.174870, -0.081298,  0.162500,   
    -0.4741,  0.3666, -0.0368,  0.3789, -0.1120,  0.2301, -0.2629,  0.9812,  # right (P5)
], dtype=np.float32)
Q_MAX = np.array([
     0.070577,  0.4085,  0.905049,  1.010016,  0.286850,  1.265453,  0.727109,  0.870000,  
     0.0123,  0.4832,  0.0859,  0.6104, -0.0506,  0.3511, -0.0061,  1.0013,  # right (P95)
], dtype=np.float32)

SPEED = 0.4
MVACC = 1.0

# Speed for moving to initial pose (slower for safety)
INIT_SPEED = 0.3
INIT_MVACC = 0.5
...
def move_to_init_pose(left, right, init_q16):
    ...
    # Switch to position mode (mode=0) so speed/mvacc are respected
    for arm in [left, right]:
        arm.set_mode(0)
        arm.set_state(0)
        time.sleep(0.1)

    # Send joint commands with controlled speed (set_servo_angle, not _j)
    ...
    qL_now = get_q7(left)
    qR_now = get_q7(right)
    print(f"[INIT] Reached. Current pose:")
    print(f"       left:  {qL_now}")
    print(f"       right: {qR_now}")
    # Stay in mode=0 (position hold) — servo mode switch happens later,
    # right before control loop starts, to avoid idle drift.
...
def main():
    ...
    # ---- Compute mean initial pose from training data ----
    ...
    # Switch to servo mode (mode=1) now, right before control loop
    for arm in [left, right]:
        arm.set_mode(1)
        arm.set_state(0)
        time.sleep(0.1)
    # Immediately send current position to hold — prevents idle drift in servo mode
    ...
    try:
        while True:
            ...
            # ---- Clamp absolute joint positions to CSV range (gripper excluded) ----
            ...
            # ---- Debug: print joint positions and dq ----
            print(f"\n[DBG] qL={qL[:3]}  qR={qR[:3]}")
            print(f"[DBG] dq_raw={dq_raw[:3]}  dq_ema={dq_ema[:3]}")
            print(f"[DBG] qL_cmd={qL_cmd[:3]}  Q_MIN_L={Q_MIN[:3]}  Q_MAX_L={Q_MAX[:3]}")

            # ---- Timing ----
            elapsed = time.perf_counter() - t0
            hz_actual = 1.0 / max(elapsed, 1e-6)
            print(f"[HZ] {hz_actual:.1f} Hz  (loop {elapsed*1000:.1f} ms)")
            if elapsed < dt:
                time.sleep(dt - elapsed)
