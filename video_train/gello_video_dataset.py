...
def _default_img_transform(img_size: int = 128):
    # torchvision transforms: resize to square -> tensor -> imagenet normalize
...
class GelloVideoDataset(Dataset):
    def __init__(
        self,
        ...
        image_col: Optional[str] = None,  # if your CSV has a column like "image_path"
        stride: int = 1,
        ...
        self.sessions = []
        self.index = []  # list of (session_idx, start_idx)

        # Default q keys (your 16D)
        if q_keys is None:
            q_keys = (
        ...
        for si, csv_path in enumerate(csvs):
            ...
            # delta_q from q[t+1]-q[t]
            dq = np.zeros_like(q, dtype=np.float32)
            dq[:-1] = q[1:] - q[:-1]
            dq[-1] = 0.0

            N = len(df)
            # We need indices such that we can take:
            # obs:  [s : s+To)
            # act:  [s : s+T)
            To = self.n_obs_steps
            T = self.horizon
            max_start = N - max(To, T)  # s+max(To,T) <= N
            if max_start < 0:
                continue
    ...
    def __getitem__(self, idx: int) -> Dict[str, object]:
        ...
        for p in img_paths:
            ...
            im = self.transform(im)  # (3,224,224)
            imgs.append(im)
        ...
        # Action: dq over horizon [s : s+T)
        action = torch.from_numpy(sess["dq"][s:s+T]).to(self.dtype)  # (T,16)
    ...
    def get_normalizer(self, mode="limits", **kwargs):
        ...
        # Collect q and dq (action) from all sessions
        all_q = []
        all_action = []
        for sess in self.sessions:
            all_q.append(torch.from_numpy(sess["q"]))        # (N, 16)
            all_action.append(torch.from_numpy(sess["dq"]))   # (N, 16)

        all_q = torch.cat(all_q, dim=0)       # (total_N, 16)
        all_action = torch.cat(all_action, dim=0)  # (total_N, 16)
